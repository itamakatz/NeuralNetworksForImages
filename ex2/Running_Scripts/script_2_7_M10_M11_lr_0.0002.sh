...
if [[ "run" == $1 || "print" == $1 ]]; then
	...
	for l in "${LOSS[@]}"; do
		...
		COUNT=$((COUNT + 1))
		if [ "run" = $1 ]; then
			python3 ./ex2.py -g $GEN -d $DIS -b $b -l $l -e $e -z $z --dataset "celeb" -k $k -gs $gs -ds $ds -sm -lr 0.0002
		fi
		if [ "print" = $1 ]; then
			echo python3 ./ex2.py -g \"$GEN\" -d \"$DIS\" -b $b -l \"$l\" -e $e -z $z --dataset "celeb" -k $k -gs $gs -ds $ds -sm  -lr 0.0002
		fi
	done
	done
	done
