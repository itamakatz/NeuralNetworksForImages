...
def print_parameter(name, val):
  '''short method to reduce code duplicity when printing the value of some variable'''
  print(f"{name}: {val}")
...
def save_checkpoint(epoch, model, optimizer, loss_func):
  ...
  checkpoint = { 
    ...
    'optimizer': optimizer,
    'loss_func': loss_func}
  saving_path = SavingPath.get_path(f"_checkpoint_epoch-{epoch+1}.pkl")
...
def general_plot(losses, dataloader_size, to_save, subdir = None):
  ...
  if(isinstance(losses[0], list)):
    ...
    plt.legend(["auto encoder losses"])
...
def save_info(args, ae, dataset, loss_func, optimizer, CPU_MODE):
  ...
  with open(SavingPath.get_path('_info.txt'), 'a+') as file:
    ...
    file.write(f"Fit-Z:\t\t\t\t\t\t\t{args.fit_z}\n")
    file.write(f"Batches:\t\t\t\t\t\t{args.batch}\n")
    ...
    file.write(f"Encoder Name:\t\t\t\t{args.e_model}\n")
    file.write(f"Decoder Name:\t\t\t\t{args.d_model}\n")
    file.write("\n")
    ...
    with redirect_stdout(file):
      file.write("Encoder:\n")
      ae.encoder.summary((args.batch, 1, 28, 28), not CPU_MODE)
      file.write("\n")
      file.write("Decoder:\n")
      ae.decoder.summary((args.batch, args.z_size), not CPU_MODE)
